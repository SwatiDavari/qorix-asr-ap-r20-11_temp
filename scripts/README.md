...
## Opening qorix-gnc/qorix-vnv read-only in VS Code

`qorix-engg.code-workspace` opens `qorix-gnc` and `qorix-vnv` as
`(read-only reference)` roots alongside this repo. Each of those sibling
repos carries its own `.vscode/settings.json` with
`"files.readonlyInclude": {"**": true}`, so VS Code blocks edits to their
files from inside the workspace — a reminder that they're owned
upstream and should be changed in their own repos, not here. This is
editor-level only; it has no effect on `sync_org_content.sh`, which
already only ever reads from them.
